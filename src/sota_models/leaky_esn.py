...
class LeakyESN:
    """
    Leaky ESN with stacked multi-step readout.

    State update:
      x <- (1-a)x + a*tanh(Win*[1;u] + W*x + b)

    Readout:
      y_multi <- Wout*[1;u;x]
    """

    def __init__(self, res_size: int, seed: int, config: LeakyESNConfig):
        if int(res_size) <= 0:
        ...
        self.Win: Optional[torch.Tensor] = None
    ...
    def _step(self, u: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        pre = self.Win @ torch.vstack((self.one, u)) + self.W @ x + self.b
        x_new = torch.tanh(pre)
        return (1.0 - self.cfg.leaky_rate) * x + self.cfg.leaky_rate * x_new
    ...
    def init_weights(self) -> None:
        ...
        Win = (torch.rand(self.res_size, 2, device=self.device, dtype=self.dtype) - 0.5) * self.cfg.input_scaling
        b = (torch.rand(self.res_size, 1, device=self.device, dtype=self.dtype) - 0.5) * self.cfg.bias_scaling

        self.W, self.Win, self.b = W, Win, b
    ...
    def train_readout(self, data_pm1: torch.Tensor, init_len: int, train_len: int) -> int:
        """
        Fit readout on the post-washout training block.

        Returns the effective training length after multi-step boundary checks.
        """
        ...
        x = torch.zeros((self.res_size, 1), device=self.device, dtype=self.dtype)

        Yt = torch.zeros((self.cfg.multi_step, train_len_eff), device=self.device, dtype=self.dtype)
        for k in range(self.cfg.multi_step):
            s = init_len + self.cfg.tau * (k + 1)
            e = s + train_len_eff
            Yt[k, :] = data_pm1[s:e]
        ...
        X = torch.zeros((1 + 1 + self.res_size, train_len_eff), device=self.device, dtype=self.dtype)
    ...
    def eval_train_block(self, data_pm1: torch.Tensor, init_len: int, train_len: int) -> float:
        """Evaluate MSE on the same training window used for readout fitting."""
        ...
        target = torch.zeros((self.cfg.multi_step, train_len_eff), device=self.device, dtype=self.dtype)
        for k in range(self.cfg.multi_step):
            s = init_len + self.cfg.tau * (k + 1)
            e = s + train_len_eff
            target[k, :] = data_pm1[s:e]
        ...
        Y = torch.zeros((self.cfg.multi_step, train_len_eff), device=self.device, dtype=self.dtype)
        for t in range(train_len_eff):
            u = data_pm1[init_len + t].view(1, 1)
            x = self._step(u, x)
            y_multi = self.Wout @ torch.vstack((self.one, u, x))
            Y[:, t] = y_multi[:, 0]
    ...
    def eval_test_block(
        ...
        """Evaluate multi-step MSE on the held-out test block."""
        if self.Wout is None:
        ...
        target = torch.zeros((self.cfg.multi_step, test_len_eff), device=self.device, dtype=self.dtype)
        for k in range(self.cfg.multi_step):
    ...
    def run(
        ...
        if out_dir:
            ...
            save_plot(
                ...
                title=f"Leaky ESN | RES={self.res_size} | seed={self.seed} | test 1-step",
            ...
            save_plot(
                ...
                title=f"Leaky ESN | RES={self.res_size} | seed={self.seed} | test first {n} 1-step",
                dpi=self.cfg.plot_dpi,
            )
            ...
            save_error_heatmap(
                ...
                title=f"Leaky ESN Error | RES={self.res_size} | seed={self.seed}",
                dpi=self.cfg.plot_dpi,
            )
