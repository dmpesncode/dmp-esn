#!/usr/bin/env python3
"""
Standalone ESN pruned once with closeness centrality.

Pure model module:
- build one ESN
- bulk prune once by closeness
- train once
...
def build_nx_graph_from_reservoir(W: torch.Tensor) -> nx.DiGraph:
    ...
    W[i, j] contributes to node i from node j in the ESN update W @ x.
    Here we store edge j -> i to represent signal flow from source neuron j
    to target neuron i.
    """
...
def compute_closeness_scores(W: torch.Tensor) -> torch.Tensor:
    """Return one closeness-centrality pruning score per reservoir node."""
...
class ClosenessPruningESN(EchoStateNetwork):
    """Single ESN that applies one bulk closeness-pruning step before training."""
    ...
    def prune_reservoir(self) -> None:
        ...
        self.node_scores = compute_closeness_scores(self.W)
    ...
    def _save_artifacts(self, y_test: torch.Tensor, out_dir: str, title_prefix: str, row: Dict) -> None:
        ...
        if self.node_scores is not None:
            ...
            pd.DataFrame(
                {
                    "node_index": node_ids,
                    "closeness_score": self.node_scores.detach().cpu().numpy(),
                    "kept": kept_lookup,
    ...
    def run(
        ...
        """Run pure bulk-closeness training and return required outputs."""
        ...
        if out_dir:
            self._save_artifacts(
                ...
                out_dir=out_dir,
                title_prefix=(
                    f"Closeness-pruned ESN | RES={self.original_reservoir_size} "
                    f"| seed={self.seed}"
                ),
                row=row,
            )
